...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isLoginMode) {
                ...
                data = await registerUser(username, email, password, displayName);
            }

            // Save token
            localStorage.setItem('token', data.token);

            // Pass the user object directly, ID normalization happens in API
            onLogin(data.user);

        } catch (err: any) {
            if (err.response && err.response.data && err.response.data.msg) {
                // Server responded with an error message (e.g. Invalid Credentials)
    ...
    return (
        ...
        <div className="relative w-full h-full bg-black rounded-[45px] border-[8px] border-black shadow-xl overflow-hidden ring-4 ring-gray-100">
        ...
        {/* Main Card */}
        <div className="bg-white border border-gray-300 flex flex-col items-center py-10 px-10">
        <h1 className="text-5xl mb-6 text-gray-800" style={{fontFamily: "'Grand Hotel', cursive"}}>IChat</h1>

        {error && <div className="text-red-500 text-xs mb-4 text-center font-semibold bg-red-50 p-2 rounded">{error}</div>}
        ...
        {!isLoginMode && (
            ...
            className="w-full text-xs bg-gray-50 border border-gray-300 rounded-[3px] px-2 py-[9px] focus:border-gray-400 focus:outline-none"
            value={username}
        ...
        className={`
            ...
            <a href="#" className="text-xs text-[#00376b] hover:text-black transition-colors">Forgot password?</a>
            </div>
            ...
            <p className="text-sm my-4 text-gray-800">Get the app.</p>
            ...
            <span>© 2024 IChat from Shadow</span>
            </div>
            </footer>
            </div>
    );
